"""G26 controlled 2x2 matrix: K={4,8}, insertion={16,20}, plus B0."""
...
MATRIX = {"K4L20": (4, 20), "K8L20": (8, 20),
          "K4L16": (4, 16), "K8L16": (8, 16)}
DEFAULT_ARMS = ("B0", *MATRIX)
# G26 retains the original configurable single-arm entry (8/18 by default).
ARMS = (*DEFAULT_ARMS, "G26")


def build_parser():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--arms", nargs="+", choices=ARMS, default=list(DEFAULT_ARMS))
    parser.add_argument("--output_dir", default="./experiment_results/g26")
    parser.add_argument("--n_epochs", type=int, default=10)
    parser.add_argument("--seed", type=int, default=1024)
    parser.add_argument("--sampler_real_ratio", type=float, default=.30)
    parser.add_argument("--num_tokens", type=int,
                        help="Custom --arms G26 only (default 8); named matrix arms are fixed")
    parser.add_argument("--insert_layer", type=int,
                        help="Custom --arms G26 only (default 18); zero-based block input")
...
def arm_settings(args, arm):
    ...
    if arm in MATRIX:
        num_tokens, insert_layer = MATRIX[arm]
    elif arm == "G26":
        num_tokens = 8 if args.num_tokens is None else args.num_tokens
        insert_layer = 18 if args.insert_layer is None else args.insert_layer
    else:
        raise ValueError(f"Unknown G26 arm: {arm}")
    return {"model_name": "effort_g26", "g26_num_tokens": num_tokens,
            "g26_insert_layer": insert_layer, "g26_mil_temperature": args.mil_temperature,
            "g26_evidence_weight": args.evidence_weight, "g26_gate_width": args.gate_width,
            "g26_aux_max_weight": args.aux_max_weight, "g26_score_mode": "gated"}


def build_arm_config(args, arm, run_dir, build_config, for_training):
    # Reuse only the B0 protocol builder, then add G26's independently named keys.
...
def evaluate_diagnostics(config, checkpoint, run_dir, utilities):
    ...
    for mode in ("cls", "evidence"):
        ...
        evaluation["g26_score_mode"] = mode
...
def run_one(args, arm, run_dir, utilities):
    ...
    result = {"exp_name": f"G26/{arm}", "arm": arm, "seed": args.seed,
    ...
    try:
        ...
        primary = utilities.evaluate_model(evaluation, checkpoint, baseline.TEST_DS,
                                           baseline.TRAIN_DS, str(run_dir), f"G26/{arm}")
...
def main(argv=None):
    ...
    custom_shape = args.num_tokens is not None or args.insert_layer is not None
    if ((custom_shape and "G26" not in args.arms)
            or ("G26" in args.arms and any(arm in MATRIX for arm in args.arms))):
        parser.error("Use --arms G26 [B0] for a custom shape; do not mix with fixed matrix arms")
    ...
    from g26_diagnostics import collect_routing_diagnostics
    ...
    paths = ["experiments/run_g26.py", "experiments/g26_diagnostics.py", "experiments/run_g25.py",
             "experiments/experiment_utils.py", "training/detectors/effort_detector_g26.py",
             "training/detectors/g26_tokens.py", "training/detectors/g25v2_tokens.py",
