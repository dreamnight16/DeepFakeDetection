...
class DataContracts(unittest.TestCase):
    ...
    def test_window_identical_across_arms(self):
        ...
        rows = [
            pair(f"r{i}", m)
            for m in DEFAULT_CONFIG["data"]["train_methods"]
            for i in range(6)
        ]
        plans = [
            make_window(rows, resolve_arm(DEFAULT_CONFIG, arm, 42), 7) for arm in "ABCD"
        ]
    ...
    def test_data_changes_over_updates_but_repeats_on_resume(self):
        ...
        rows = [
            pair(f"r{i}", m) for m in cfg["data"]["train_methods"] for i in range(8)
        ]
