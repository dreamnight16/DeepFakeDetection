#!/bin/bash
# ===========================================================================
# Sweep: Laplacian Pyramid Full-Scope Hard Label (label × full)
#
#   Two configs:
#     1) lap_pyramid_label0_full — all Laplacian levels mixed, RF label→0
#     2) lap_pyramid_label1_full — all Laplacian levels mixed, RF label→1
#
#   Fixed: gamma=1.0, alpha=5.0, trainer_v2, v1 sampler (real_ratio=0.30)
# ===========================================================================
# Usage:
#   bash sweep_pyramid_label_full.sh              # single GPU
#   bash sweep_pyramid_label_full.sh 4            # 4-GPU DDP (torchrun)
set -euo pipefail

YAML="./training/config/detector/effort.yaml"
LOG_DIR_BASE="./zhiyuanyan/logs/benchv2/icml25/pyramid_label_full_sweep"
SWEEP_LOG="sweep_pyramid_label_full.log"
...
TRAIN_PY="./training/train_pyramid_label_full_sweep.py"
...
run_one() {
    ...
    local TRAIN_LOG="sweep_pyramid_label_full_${SAFE_TAG}_train.log"
    echo "===== $TAG ====="

    TMP_YAML=$(mktemp /tmp/effort_pyramid_label_full_XXXXXX.yaml)
    cp "$YAML" "$TMP_YAML"
...
# ═══════════════════════════════════════════════════════════════════════════
# 2×1 Experiment: label_mode × scope=full
# ═══════════════════════════════════════════════════════════════════════════
RUNS=(
    "label0_full    lap_pyramid_label0_full"
    "label1_full    lap_pyramid_label1_full"
)

TOTAL=${#RUNS[@]}
echo "============================================================" | tee -a "$SWEEP_LOG"
echo "  Pyramid Full-Scope Hard Label — $TOTAL configs"            | tee -a "$SWEEP_LOG"
echo "  Factors: hard_label ∈ {0, 1} × scope = full"               | tee -a "$SWEEP_LOG"
...
echo "" | tee -a "$SWEEP_LOG"
echo "============================================================" | tee -a "$SWEEP_LOG"
echo "  Summary — Pyramid Full-Scope Hard Label  ($(date '+%Y-%m-%d %H:%M'))" | tee -a "$SWEEP_LOG"
echo "============================================================" | tee -a "$SWEEP_LOG"
