...
def choose_optimizer(model, config):
    ...
    # optional wrapper: 'sam' wraps base optimizer with SAM; 'pcgrad' wraps with PCGrad
    wrapper = config.get('optimizer_wrapper', None)
    if wrapper == 'sam':
        # re-build SAM using the already-constructed base optimizer's class and defaults
        # rho must be passed separately to avoid leaking into base_optimizer kwargs
        base_defaults = {k: v for k, v in optimizer.defaults.items()}
        optimizer = SAM(model.parameters(), type(optimizer),
                        rho=config.get('sam_rho', 0.05), **base_defaults)
    elif wrapper == 'pcgrad':
...
def main():
    ...
    # ── Asymmetric Mixup CLI override ─────────────────────────────────────────
    if args.mixup_gamma is not None:
        config['use_mixup'] = True
        config['mixup_gamma'] = args.mixup_gamma
    # ─────────────────────────────────────────────────────────────────────────
    if config['lmdb']:
